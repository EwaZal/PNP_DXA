...
# Ważne: wykres porownujacy TBS w grupie badanej i kontrolnej pomiedzy osobami bez i ze złamaniami - w grupie osób z PHPT i złamaniami w wywiadzie stwierdzono istotnie niższe TBS w porównaniu do osób z PHPT bez złamań w wywiadzie.
...
# Ważne: Czy jest zwiazek miedzy zlamaniami w wywiadzie a TBS <1.2 i >1.2? - TAK, wychodzi, ze jest wiecej zlaman u osob z nizszym TBS niż z wyższym:
```{r}
...
# Logarytm z Ca a BMD w grupie badanej
```{r}
...
# Logarytm z Ca a Logarytm z BMD w grupie badanej
```{r}
...
# Ca a logarytm z BMD w grupie badanej
```{r}
...
```
# iPTH a BMD - w grupie badanej i kontrolnej
```{r}
ggplot(PHPT, aes(iPTH, BMD_neck)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
# logarytm z iPTH a BMD szyjki kości udowej
```{r}
...
# logarytm z PTH a BMD z podzialem na grupy badaną i kontrolną
```{r}
ggplot(PHPT, aes(log_iPTH, BMD_neck)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
# z MOF_risk
# Ca a ryzyko złamania głównego (MOF_risk)
```{r}
...
# z MOF_risk
# iPTH a  ryzyko złamania głównego
```{r}
...
# TBS a MOF risk w grupie badanej i kontrolnej
```{r}
ggplot(PHPT, aes(TBS, MOF_risk)) +
  geom_point() +
  geom_smooth(method = "lm")  +
...
# BMD a MOF risk w grupie badanej i kontrolnej
```{r}
...
# BMD a logarytm z HF risk w grupach badanej i kontrolnej 
```{r}
ggplot(PHPT, aes(BMD_neck, log_HF)) +
  ...
  stat_cor(method = "spearman")
```
# VFA w grupach badanej i kontrolnej?
```{r}
...
PHPT_clean <- PHPT %>%
  filter(VFA %in% c("fracture","norm")) %>%
  droplevels()

mosaicplot(~ group + VFA, data = PHPT_clean, color = TRUE)

tab <- table(PHPT_clean$group, PHPT_clean$VFA)
tab
fisher.test(tab)


OddsRatio(tab, method = "wald", conf.level = 0.95)

```
# ile jest złamań wszystkich?
```{r}
...
PHPT %>%
  count(group, fracture_any) %>%
  group_by(group) %>%
  mutate(percent = 100 * n / sum(n))

mosaicplot(~ group + fracture_any, data = PHPT, color = TRUE)

fisher.test(table(PHPT$group, PHPT$fracture_any))

pairwise_fisher_test(table(PHPT$group, PHPT$fracture_any), p.adjust.method = "fdr")
```
# ile jest złamań NVF?
```{r}
PHPT %>%
...
PHPT %>%
  count(group, fracture) %>%
  group_by(group) %>%
  mutate(percent = 100 * n / sum(n))
```
# ile jest złamań VF (na podstawie VFA)?
```{r}
PHPT %>%
  group_by(group) %>%
  count(VFA)
...
PHPT %>%
  ...
  mutate(percent = 100 * n / sum(n))
```
# Jakie jest TBS u pacjentów bez i z złamaniem?
```{r}
PHPT_B %>%
  group_by(fracture_any) %>%
  get_summary_stats(TBS)

wilcox.test(TBS ~ fracture_any, data = subset(PHPT_B, group != "NA"))
