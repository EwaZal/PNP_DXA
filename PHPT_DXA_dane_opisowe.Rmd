...
ggplot(PHPT, aes(x = fracture, y = TBS, fill = fracture)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values = c("gray89", "gray27")) +
  scale_x_discrete(labels= c("without fracture", "with fracture")) +
  labs(x = NULL, y = "Trabecular bone score") +
  stat_compare_means(method = 'wilcox.test', label = "p.format", size = 4) +
  theme_bw() +
...
# czy jest zwiazek miedzy zlamaniami w wywiadzie a TBS <1.2 i >1.2? - TAK, wychodzi, ze jest wiecej zlaman u osob z nizszym TBS niż z wyższym:
```{r}
PHPT_B <- PHPT %>%
  filter(group == 'B')
...
pairwise_fisher_test(table(PHPT$fracture, PHPT$TBS_split), p.adjust.method = "fdr")
