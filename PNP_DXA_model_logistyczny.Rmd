...
``` {r}
model <- glm(fracture ~ group + sex + age + nicotine_dependence + Ca + iPTH + Tscore_femur_neck + Tscore_femur_total + Tscore_lumbar + TBS, data = PHPT_ml, family = binomial)
summary(model)
# OR
exp(coef(model))
...
```{r}
model6 <- glm(fracture ~ age + sex + group + TBS + nicotine_dependence + Ca + iPTH, data = PHPT_ml, family = binomial)
summary(model6)
exp(coef(model6))
exp(confint(model6))
...
```{r}
model7 <-  glm(fracture ~ age + sex + group + TBS + nicotine_dependence + Ca + iPTH + BMD_neck, data = PHPT_ml, family = binomial)
summary(model7)
exp(coef(model7))
exp(confint(model7))
...
```{r}
model8 <- glm(fracture ~ age + sex + TBS + nicotine_dependence + Ca + iPTH + Tscore_femur_neck + Tscore_femur_total + Tscore_lumbar, data = PHPT_ml, family = binomial)
summary(model8)
exp(coef(model8))
exp(confint(model8))
...
```{r}
model9 <- glm(fracture ~ age + sex + TBS + Tscore_femur_neck + Tscore_femur_total + Tscore_lumbar, data = PHPT_ml, family = binomial)
summary(model9)
exp(coef(model9))
exp(confint(model9))
