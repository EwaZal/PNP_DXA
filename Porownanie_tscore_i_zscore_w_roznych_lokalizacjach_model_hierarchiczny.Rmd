...
ggplot(emm_df, aes(x = site, y = emmean, color = group, group = group)) +
  geom_point(size = 3, position = pd) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.14, position = pd) +
  geom_line(alpha = 0.7, position = pd) +
  labs(
    x = "Lokalizacja",
    y = "T-score (średnie marginalne ±95% CI)",
    color = "Grupa",
    title = "T-score wg lokalizacji i grupy (model mieszany z efektem losowym dla pacjenta)"
  ) +
